...
#include "../Utilities/DubinsUtilities.h"
    ...
    namespace service {
        ...
        //example: if (afrl::cmasi::isServiceStatus(receivedLmcpMessage->m_object.get()))
        {
            if (uxas::messages::route::isRoutePlanRequest(receivedLmcpMessage->m_object.get())) {
                ...
                m_airVehicles.insert(id);
                calculateTurnRadius(std::dynamic_pointer_cast<afrl::cmasi::AirVehicleConfiguration>(receivedLmcpMessage->m_object));
        ...
        RoutePlannerService::HandleRoutePlanRequestMsg(std::shared_ptr<uxas::messages::route::RoutePlanRequest> request) {
            ...
            if (state != m_entityStates.end()) {
                ...
                vehicleHeading = state->second->getHeading();
                                ...
                                if (vv != s->second.end()) {
                                    ...
                                    if (startPt.in(*env, 1e-4) && endPt.in(*env, 1e-4)) {
                                        ...
                                        if (!request->getIsCostOnlyRequest()) {
                                            ...
                                            //path already has the waypoints with coordinates already in north and east positions????
                                            //do dubin logic here

                                            //use the planned waypoints and calculate dubins paths
                                            bool dubinsTest = true; //Remove this after tested
                                            if (dubinsTest == true) {
                                                double turnRadius = 0.0; //if radius not found in m_turnRadii, then keep at 0
                                                if (m_turnRadii.count(vehicleId) > 0) {
                                                    turnRadius = m_turnRadii[vehicleId];
                                                }
                                                size_t planWaypointNumber = 1;
                                                double endHeading;
                                                uxas::common::utilities::DubinsConfiguration startConfig, endConfig;
                                                //special case for first waypoint and last waypoint. 
                                                // n = index of path waypoints (before dubin's path math)
                                                //1) FOR THE CASE OF THE FIRST WAYPOINT (n == 0):
                                                //   - first add first waypoint to path
                                                //   - calculate the start heading for the first waypoint using pt1(north reference of vehicle), pt2(vehicle pt), and pt3(waypoint[n]))
                                                //   - use the start heading to create the start config
                                                //   This is the same calculation in every other waypoint for end heading...
                                                //   - calculate the heading of the second waypoint, use this as end heading p1(north reference of waypoint[n]), pt2(waypoint[n]), pt3(waypoint[n + 1])
                                                //   - then do dubin's path with two configs, convert to cmasi waypoints, and add to the plan
                                                //
                                                //2) FOR THE CASE OF THE LAST WAYPOINT (n == path.size() - 1) 
                                                //   - add last waypoint directly from path to plan
                                                //
                                                //3) EVERY OTHER WAYPOINT (until n == path.size() - 1)
                                                //   - use start heading as end heading from previous calculation
                                                //   - use start heading and waypoint n - 1 as x and y in start configuration
                                                //   - calculate the end heading using p1(north reference of waypoint[n]), pt2(waypoint[n]), pt3(waypoint[n + 1])
                                                //   - use end heading and waypoint n as x and y in end configuration
                                                //   - then make a dubin's path with configs
                                                //   - convert dubin's path's waypoints to cmasi waypoints
                                               
                                                for (size_t n = 0; n < path.size() - 1; n++) {
                                                    if(n == 0)
                                                    {
                                                        //always start with vehicle's location? The vehicle's heading is its bearing angle (cw +). subtract from 2pi to get ccw +
                                                        auto startHeading = n_Const::c_Convert::dTwoPi() - n_Const::c_Convert::toRadians(vehicleHeading);
                                                        startConfig = uxas::common::utilities::DubinsConfiguration(vehiclePt.y(), vehiclePt.x(), startHeading);
                                                    } else 
                                                    {
                                                        startConfig = uxas::common::utilities::DubinsConfiguration(path[n-1].y(), path[n-1].x(), endHeading);
                                                    }
                                                    
                                                    auto northRef = VisiLibity::Point(path[n].x(), path[n].y() + 100);
                                                    endHeading = VisiLibity::Point::angle_separation(northRef, path[n], path[n+1]).get();
                                                    endConfig = uxas::common::utilities::DubinsConfiguration(path[n].y(), path[n].x(), endHeading);
                                                    auto dubinsWaypoints = uxas::common::utilities::DubinsPath(startConfig, endConfig, turnRadius).getDubinsWaypoints();
                                                    for(size_t m = 0; m < dubinsWaypoints.size(); m++)
                                                    {
                                                        //add waypoint
                                                        wp = new afrl::cmasi::Waypoint();
                                                        double lat, lon;
                                                        flatEarth.ConvertNorthEast_mToLatLong_deg(dubinsWaypoints[m].y, dubinsWaypoints[m].x, lat, lon);
                                                        wp->setLatitude(lat);
                                                        wp->setLongitude(lon);
                                                        wp->setAltitude(alt);
                                                        wp->setAltitudeType(altType);
                                                        wp->setNumber(planWaypointNumber);
                                                        wp->setNextWaypoint(planWaypointNumber + 1);
                                                        wp->setSpeed(speed);
                                                        wp->setTurnType(afrl::cmasi::TurnType::TurnShort);
                                                        plan->getWaypoints().push_back(wp);
                                                        
                                                        planWaypointNumber = planWaypointNumber + 1;      
                                                    }
                                                ...
                                                wp = new afrl::cmasi::Waypoint();
                                                double lat, lon;
                                                flatEarth.ConvertNorthEast_mToLatLong_deg(path[path.size() - 1].y(), path[path.size() - 1].x(), lat, lon);
                                                wp->setLatitude(lat);
                                                wp->setLongitude(lon);
                                                wp->setAltitude(alt);
                                                wp->setAltitudeType(altType);
                                                wp->setNumber(planWaypointNumber);
                                                wp->setNextWaypoint(planWaypointNumber);
                                                wp->setSpeed(speed);
                                                wp->setTurnType(afrl::cmasi::TurnType::TurnShort);
                                                plan->getWaypoints().push_back(wp);
                                                
                                            } else { //not using dubins waypoints
        ...
        void RoutePlannerService::calculateTurnRadius(const std::shared_ptr<afrl::cmasi::AirVehicleConfiguration>& airVehicleConfiguration) {
            ...
            double dTanMaxBankAngle = std::tan(nominalMaxBankAngle_rad);
            double turnRadius_m = (dTanMaxBankAngle <= 0.0) ? (0.0) : (std::pow((nominalSpeed_mps), 2) / (n_Const::c_Convert::dGravity_mps2() * dTanMaxBankAngle));

            //Should there be a turn radius offset? The route planner visibility service has this in xml node
            //turnRadius_m += m_turnRadiusOffset_m;
            m_turnRadii[airVehicleConfiguration->getID()] = turnRadius_m;
        }
